...
public class PlayerMovement : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D col) {
        if (col.gameObject.CompareTag("Item") || col.gameObject.CompareTag("PuzzlePoint") || col.gameObject.CompareTag("Status")) {
            if (Input.GetKey("i") && !hasInteracted && uiActive == -1) { // If 'i' is pressed
                ...
                } else if (col.gameObject.CompareTag("Status")) {
            ...
            if (col.gameObject.CompareTag("Status") || !col.gameObject.GetComponent<IndexSetter>().isSolved) { 
    ...
    void OnTriggerExit2D(Collider2D col) {
        if (col.gameObject == choiceDatabase.Instance.returnPortal && choiceDatabase.Instance.returnPortal.GetComponent<BoxCollider2D>().isTrigger && !secondPortalTriggered) {
            ...
            Destroy(choiceDatabase.Instance.noGoingBackBarriers.GetChild(0).gameObject); // Removes noGoingBackBarrier
