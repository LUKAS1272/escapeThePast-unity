...
public class ChooseButton : MonoBehaviour
{
    ...
    public void ChoiceMade() {
        ...
        if (!isAvailable) {
            StartCoroutine(ongoingErrorMessages.Instance.writeError(gameObject.transform.parent.transform.parent.GetChild(1).GetChild(gameObject.transform.GetSiblingIndex()).transform.GetComponent<Text>().text)); // Gets the label of pressed button
        } else {
            switch (menuId) {
                case 0: // Prison doors
                    ...
                    switch (choiceIndex) {
                        case 0: // redPot
                            ...
                            for (int i = 0; i < 2; i++) {
                                ...
                                guards.GetChild(i).GetChild(0).GetComponent<SpriteRenderer>().sprite = choiceDatabase.Instance.skull; // Updates status to dead
                                guards.GetChild(i).GetChild(1).gameObject.SetActive(false); // Removes vision field
                                guards.GetChild(i).GetComponent<SpriteRenderer>().color = new Color32(255, 0, 0, 255); // Changes color
                            }
                        ...
                        case 1: // bluePot
                            ...
                            for (int i = 0; i < 2; i++) {
                                Transform guards = choiceDatabase.Instance.guards[0].transform;
                                guards.GetChild(i).GetChild(0).GetComponent<SpriteRenderer>().sprite = choiceDatabase.Instance.sleep; // Updates status to sleeping
                                guards.GetChild(i).GetChild(1).gameObject.SetActive(false); // Removes vision field
                                guards.GetChild(i).GetComponent<SpriteRenderer>().color = new Color32(255, 255, 0, 255); // Changes color
                            }
                        ...
                        case 2: // greenPot
                            ...
                            for (int i = 0; i < 2; i++) {
                                Transform guards = choiceDatabase.Instance.guards[0].transform;
                                guards.GetChild(i).GetChild(0).GetComponent<SpriteRenderer>().sprite = choiceDatabase.Instance.questionMark; // Updates status to clueless
                                guards.GetChild(i).GetChild(1).GetComponent<SpriteRenderer>().color = new Color32(0, 0, 0, 100); // Changes vision field color
                                guards.GetChild(i).GetChild(1).GetComponent<PolygonCollider2D>().isTrigger = true; // Turns vision field into trigger (so player can walk through it)
                                guards.GetChild(i).GetComponent<SpriteRenderer>().color = new Color32(0, 0, 0, 255); // Changes color
                            }
                ...
                case 5: // Destroying fence
                    PuzzleGenerator.Instance.SetValues(13);
                    break;
            }
    ...
    // // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // public int ongoingErrorMessages = 0;
