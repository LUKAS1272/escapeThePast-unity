...
public class PlayerMovement : MonoBehaviour
{
    ...
    void FixedUpdate() {
        ...
        for (int i = 0; i < uis.transform.childCount; i++) {
            if (uis.transform.GetChild(i).gameObject.activeInHierarchy && i != uis.transform.childCount - 1) {
                doUpdate = false;
                break;
            }
        }
    ...
    void OnTriggerStay2D(Collider2D col) {
        ...
        // if (col.gameObject.CompareTag("PuzzlePoint") && col.gameObject.GetComponent<IndexSetter>().isSolved && col.gameObject.activeInHierarchy) {
        //     interactText.SetActive(false);
        // }
    }
